...
|---|---|---|---|---|
| 1 | **Answer one question for Pard: which macOS session was One Job archived from?** You say `xian@pobox.com` is already signed into Xcode on Amber, and One Job shipping proves it. Pard's `xcodebuild` reads `No Accounts` every cycle. Both can be true: Xcode accounts are per-macOS-user, so if his headless session isn't your GUI login, it can't see your account. He's been sent the commands to distinguish that from a genuinely missing sign-in. **If it's a session problem, no sign-in is needed** — the archive runs in your GUI session, or you do it at the desk once the build is green. | one reply to Pard, or ~2 min | archive → TestFlight → Dan's prototype → submission |
...
- **The bundle ID in `project.yml` was wrong — caught by xian's question, fixed at HEAD.** The shipped 1.x identifier is **`com.longskymedia.optilisten`** (from `ios/ListenUp.xcodeproj/project.pbxproj` in the 1.x source), not `com.optilisten.ios` from Austin's 2022 setup email. An archive under the wrong ID would not have updated the existing App Store record. Pard must `xcodegen generate` before the next build.
- **Bundle-ID ownership is not an issue.** The reverse-domain string is a naming convention with no bearing on which Apple team owns the identifier; `com.longskymedia.optilisten` is registered to `YZ4B34YGX9` (1.x `DEVELOPMENT_TEAM`, and App Store Connect shows `xian@pobox.com` as Account Holder on that team). Dan's Apple ID is not involved.
